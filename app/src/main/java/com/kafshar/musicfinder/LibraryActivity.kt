...
class LibraryActivity : Activity() {
    ...
    private fun createLayout() {
        ...
        val categoriesButton = TextView(this).apply {
            text = "☷"
            textSize = 26f
            setTextColor(0xFF20C9C9.toInt())
            gravity = Gravity.CENTER
            contentDescription = "Music Categories"
            setOnClickListener {
                startActivity(Intent(this@LibraryActivity, CategoriesActivity::class.java))
            }
        }
        header.addView(categoriesButton, LinearLayout.LayoutParams(55, 55))
    ...
    private fun loadLibrary() {
        ...
        songs.take(200).forEachIndexed { index, song -> addSongView(song, index) }
    }
    ...
    private fun loadCover(url: String, target: ImageView) {
        ...
        executor.execute {
            ...
            try {
                connection = URL(url).openConnection() as? HttpURLConnection ?: return@execute
                ...
                if (bitmap != null && !destroyed) {
                    ...
                    runOnUiThread { if (!destroyed) target.setImageBitmap(bitmap) }
                }
            } catch (_: Exception) {
            } finally {
    ...
    private fun playSong(song: SongResult) {
        ...
        try {
            ...
            Toast.makeText(this, "در حال پخش: ${song.title}", Toast.LENGTH_SHORT).show()
        } catch (_: Exception) {
            Toast.makeText(this, "پخش آهنگ انجام نشد", Toast.LENGTH_SHORT).show()
        }
    }
    ...
    private fun deleteSong(song: SongResult) {
        try {
            ...
            Toast.makeText(this, "از کتابخانه حذف شد", Toast.LENGTH_SHORT).show()
            loadLibrary()
        } catch (_: Exception) {
            Toast.makeText(this, "حذف انجام نشد", Toast.LENGTH_SHORT).show()
        }
    }
