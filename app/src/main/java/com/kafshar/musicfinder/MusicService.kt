...
class MusicService : MediaSessionService() {
    ...
    override fun onCreate() {
        ...
        player.addListener(object : Player.Listener {
            override fun onIsPlayingChanged(isPlaying: Boolean) {
                safeSendUpdate(); updateForegroundNotification()
            }
        ...
        if (session != null) {
            builder.setStyle(
                MediaStyleNotificationHelper.MediaStyle(session)
                    .setShowActionsInCompactView(intArrayOf(1, 2, 3))
            )
        }
    ...
    private fun getCurrentTitle(): String = try {
        player.currentMediaItem?.mediaMetadata?.title?.toString()?.ifBlank { "Music Finder" } ?: "Music Finder"
    } catch (_: Exception) { "Music Finder" }

    private fun getCurrentArtist(): String = try {
        player.currentMediaItem?.mediaMetadata?.artist?.toString()?.ifBlank { "Music Finder" } ?: "Music Finder"
    } catch (_: Exception) { "Music Finder" }

    private fun updateForegroundNotification() {
        if (released || mediaSession == null) return
        try {
            getSystemService(NotificationManager::class.java)?.notify(NOTIFICATION_ID, buildForegroundNotification())
        } catch (_: Exception) { }
    }
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        try {
            when (intent?.action) {
                ...
                ACTION_PAUSE -> { player.pause(); safeSendUpdate(); updateForegroundNotification() }
                ACTION_TOGGLE -> {
                ...
                ACTION_SET_VOLUME -> setSystemVolume(intent.getIntExtra(EXTRA_VOLUME, currentSystemVolumePercent()).coerceIn(0, 100))
    ...
    private fun setSystemVolume(percent: Int) {
        try {
            ...
            player.volume = 1f
            getSharedPreferences("player_settings", MODE_PRIVATE).edit().putInt("volume_percent", currentSystemVolumePercent()).apply()
            safeSendUpdate()
        } catch (_: Exception) { }
    }
    ...
    private fun playUrl(intent: Intent) {
        ...
        if (url.isNullOrBlank() || !ServerConfig.isAllowedMediaUrl(url)) { safeSendUpdate(); return }
        ...
        try {
            ...
            if (queue.size > 1) player.setMediaItems(queue, 0, 0L) else player.setMediaItem(current)
            player.prepare(); player.play(); saveToHistory(current); safeSendUpdate(); updateForegroundNotification()
        } catch (_: Exception) {
            try { player.stop(); player.clearMediaItems() } catch (_: Exception) { }
            safeSendUpdate(); updateForegroundNotification()
        }
    ...
    private fun next() {
        try {
            if (player.hasNextMediaItem()) player.seekToNextMediaItem() else if (player.currentMediaItem != null) player.seekTo(0, 0L)
            player.prepare(); player.play(); safeSendUpdate(); updateForegroundNotification()
        } catch (_: Exception) { safeSendUpdate() }
    }
    ...
    private fun buildRelatedQueue(currentUrl: String, currentTitle: String, currentArtist: String, currentCover: String): List<MediaItem> {
        ...
        try {
            ...
            fun score(song: SongResult): Int {
                var score = 0
                val a = song.artist.lowercase(); val t = song.title.lowercase()
    ...
    private fun createMediaItem(url: String, title: String, artist: String, cover: String): MediaItem {
        val metadata = MediaMetadata.Builder().setTitle(title.ifBlank { "Music Finder" }).setArtist(artist.ifBlank { "Music Finder" }).apply {
            if (cover.isNotBlank()) try { setArtworkUri(Uri.parse(cover)) } catch (_: Exception) { }
        }.build()
        return MediaItem.Builder().setUri(url).setMediaId(url).setMediaMetadata(metadata).build()
    }
    ...
    private fun saveToHistory(item: MediaItem) {
        try {
            ...
            val lines = old.split("\n").filter { it.isNotBlank() && !it.startsWith("$url|||") }.toMutableList()
            lines.add(0, line)
    ...
    private fun seekPercent(percent: Int) {
        try {
            ...
            player.seekTo(duration * percent.coerceIn(0, 100) / 100L)
            safeSendUpdate()
        } catch (_: Exception) { }
    }
    ...
    private fun createOpenAppPendingIntent(): PendingIntent {
        ...
        return PendingIntent.getActivity(this, 200, intent, PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)
    }

    override fun onGetSession(controllerInfo: MediaSession.ControllerInfo): MediaSession? = mediaSession

    override fun onTaskRemoved(rootIntent: Intent?) {
        try { if (::player.isInitialized && player.playbackState != Player.STATE_IDLE) player.play() } catch (_: Exception) { }
    ...
    override fun onDestroy() {
        ...
        mediaSession = null
        try { if (::player.isInitialized) { player.stop(); player.clearMediaItems(); player.release() } } catch (_: Exception) { }
        try { stopForeground(STOP_FOREGROUND_REMOVE) } catch (_: Exception) { }
        super.onDestroy()
    }
