...
class MusicService : MediaSessionService() {
    ...
    override fun onCreate() {
        ...
        // The service must enter foreground immediately. Do this before any
        // network/media work so Android's foreground-start deadline is met.
    ...
    private fun setVolumePercent(percent: Int) {
        ...
        try {
            val max = audioManager
                .getStreamMaxVolume(AudioManager.STREAM_MUSIC)
                .coerceAtLeast(1)
            val raw = kotlin.math.round(max * value / 100f)
                .toInt()
                .coerceIn(0, max)
    ...
    override fun onStartCommand(
        ...
        try {
            when (intent?.action) {
                ...
                ACTION_PREVIOUS -> {
                    if (::player.isInitialized && player.hasPreviousMediaItem) {
                ...
                ACTION_SEEK_PERCENT -> {
                    if (::player.isInitialized) {
                        ...
                        if (duration > 0 && duration != C.TIME_UNSET) {
                            val target = (
                                duration.toDouble() * percent.toDouble() / 100.0
    ...
    override fun onTaskRemoved(rootIntent: Intent?) {
        // Keep playback alive when the task is removed; Android's media service
        // lifecycle controls whether the service should ultimately be stopped.
