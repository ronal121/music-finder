...
class MusicService : MediaSessionService() {
    ...
    override fun onCreate() {
        ...
        player = ExoPlayer.Builder(this)
            ...
            .build()
        player.volume = 1f
        mediaSession = MediaSession.Builder(this, player).setSessionActivity(openAppPendingIntent()).build()
        player.addListener(object : Player.Listener {
            ...
            override fun onPlayerError(error: PlaybackException) {
                ...
                } else if (uri.isNotBlank()) player.pause()
    ...
    private fun loadAndPlay(url: String, title: String, artist: String, cover: String) {
        if (released || url.isBlank()) return
        if (!ServerConfig.isAllowedMediaUrl(url)) { publish("Unsupported media source"); return }
        retryingUri = url
        val metadata = MediaMetadata.Builder().setTitle(title.ifBlank { "Music Finder" }).setArtist(artist.ifBlank { "Unknown Artist" }).apply { if (cover.isNotBlank()) setArtworkUri(android.net.Uri.parse(cover)) }.build()
    ...
    private fun unmute() { muted = false; setVolumePercent(previousVolume.coerceIn(1, 100)) }
    ...
    private fun createNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            getSystemService(NotificationManager::class.java)?.createNotificationChannel(NotificationChannel(CHANNEL_ID, "Music Playback", NotificationManager.IMPORTANCE_LOW).apply {
    ...
    private fun startPlaybackForeground() {
        ...
        try {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) startForeground(NOTIFICATION_ID, notification, ServiceInfo.FOREGROUND_SERVICE_TYPE_MEDIA_PLAYBACK) else startForeground(NOTIFICATION_ID, notification)
            foregroundStarted = true
        } catch (_: Exception) { foregroundStarted = false }
    }
    private fun ensureForeground() { if (!foregroundStarted && !released) startPlaybackForeground() }
    ...
    private fun actionPendingIntent(action: String, requestCode: Int): PendingIntent = PendingIntent.getService(this, requestCode, Intent(this, MusicService::class.java).setAction(action), PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)
    private fun openAppPendingIntent(): PendingIntent = PendingIntent.getActivity(this, 100, Intent(this, MainActivity::class.java).apply { flags = Intent.FLAG_ACTIVITY_SINGLE_TOP or Intent.FLAG_ACTIVITY_CLEAR_TOP }, PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)
        ...
        try {
            when (intent?.action) {
                ACTION_PLAY -> {
                    ...
                    if (url.isNotBlank()) loadAndPlay(url, intent.getStringExtra(EXTRA_TITLE).orEmpty(), intent.getStringExtra(EXTRA_ARTIST).orEmpty(), intent.getStringExtra(EXTRA_COVER).orEmpty()) else if (::player.isInitialized) player.play()
                }
                ...
                ACTION_TOGGLE -> {
                    ...
                    if (url.isNotBlank() && currentUrl != url) loadAndPlay(url, intent.getStringExtra(EXTRA_TITLE).orEmpty(), intent.getStringExtra(EXTRA_ARTIST).orEmpty(), intent.getStringExtra(EXTRA_COVER).orEmpty()) else if (player.isPlaying) player.pause() else player.play()
                }
                ACTION_STOP -> {
                ...
                ACTION_NEXT -> if (::player.isInitialized) player.seekToNextMediaItem()
                ACTION_PREVIOUS -> if (::player.isInitialized) player.seekToPreviousMediaItem()
                ...
                ACTION_SEEK_PERCENT -> {
                    if (::player.isInitialized) {
                        val raw = intent.extras?.get(EXTRA_PERCENT)
                        val p = (raw as? Number)?.toFloat()?.coerceIn(0f, 100f) ?: 0f
                        val d = player.duration
                        if (d > 0 && d != C.TIME_UNSET) {
                            val target = (d.toDouble() * p.toDouble() / 100.0).toLong().coerceIn(0L, d)
