...
object MusicPageParser {
    data class ParsedPage(val title: String, val artist: String, val cover: String, val audioCandidates: List<String>)
    ...
    fun parse(html: String, pageUrl: String): ParsedPage {
        ...
        return ParsedPage(title.trim().take(300), artist.trim().take(200), cover.trim(), candidates.take(50))
    }
    ...
    private fun normalizeUrl(raw: String, pageUrl: String): String? = try {
        ...
        val result = if (u.scheme.isNullOrBlank()) Uri.parse(pageUrl).buildUpon().appendEncodedPath(clean.removePrefix("/")).build() else u
        result.toString().takeIf { it.startsWith("http://", true) || it.startsWith("https://", true) }
